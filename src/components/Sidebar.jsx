...
const Sidebar = ({openModal}) => {
    ...
    return (
        <div className="bg-[#F8FAFC] h-full p-4 border-r md:fixed block md:w-[19vw] w-[55vw]">
            ...
            <div className="mt-5 hidden md:block relative inline-flex group w-full">
                <div
                    className="w-full absolute transitiona-all duration-1000 opacity-70 -inset-px bg-gradient-to-r from-[#44BCFF] via-[#FF44EC] to-[#FF675E] rounded-xl blur-lg filter group-hover:opacity-100 group-hover:-inset-1 group-hover:duration-200">
                </div>
                <button href="" target="" title="" role="button"
                    onClick={openModal}
                    className="w-full relative  items-center justify-center px-5 py-2 text-base font-semibold text-black transition-all duration-200 bg-[#F8FAFC] border-dashed border-2 border-transparent focus:outline-none hover:bg-gray-300 rounded-xl text-center">
                    <div className="flex justify-center gap-2">
                        {/* <i className="ri-google-play-fill text-3xl"></i> */}
                        {/* <i className="ri-app-store-fill  text-3xl"></i> */}
                    </div>
                    Connect to Wallet
                </button>
            </div>
